// user
export const GET_USER_MONKEYS = 'GET_USER_MONKEYS';
export const GET_USER_CUPCAKES = 'GET_USER_CUPCAKES';
export const ADD_USER_CUPCAKE = 'ADD_USER_CUPCAKE';
export const ADD_USER_MONKEY = 'ADD_USER_MONKEY';

export const SET_BAKER = 'SET_BAKER';

// auth
export const AUTH_ERROR = 'AUTH_ERROR';

// auth user
export const AUTH_USER = 'AUTH_USER';
export const UNAUTH_USER = 'UNAUTH_USER';
export const FETCH_USER = 'FETCH_USER';

// auth baker
export const AUTH_BAKER = 'AUTH_BAKER';
export const UNAUTH_BAKER = 'UNAUTH_BAKER';
export const FETCH_BAKER = 'FETCH_BAKER';
